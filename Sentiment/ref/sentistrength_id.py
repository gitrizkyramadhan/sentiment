...
class sentistrength:
    ...
    def senti(self,term):
        try:
            diff = dl.Differ()
            matches = dl.get_close_matches(term,[line[:line.find(':')] for line in open("sentiwords_id.txt").read().splitlines()])
            # print matches
            a = diff.compare(term,matches[0])
            seq = dl.SequenceMatcher(None,term,matches[0])
            r = seq.ratio()
            print r
            print "".join(a)
            print "term",term
            ...
            # ratio = levi.ratio(term,matches[0])
            ratio = r
            if ratio >= 0.7:
                term = matches[0]
            else:
    ...
    def emosikon(self,term):
        try:
            # matches = dl.get_close_matches(term, self.emoticon_txt)
            # term = matches[0]
    ...
    def ungkapan(self,term):
        try:
            # matches = dl.get_close_matches(term, self.idioms_txt)
            # term = matches[0]
    ...
    def booster(self, term):
        try:
            # term = dl.get_close_matches(term, [line.replace('\n', '') for line in open("boosterwords_id.txt").read().splitlines()])
...
print senti.main("agnezmo pntr dan cantik sekali tetapi lintah darat :)")


print [line.replace(line[line.find(':'):],"").replace('\n','').split(":") for line in open("sentiwords_id.txt").read().splitlines()]

print [line[:line.find(':')] for line in open("sentiwords_id.txt").read().splitlines()]
